...
if test ! -d "provider/cmd/pulumi-tfgen-redpanda"; then
  echo "Project already renamed, provider/cmd/pulumi-tfgen-redpanda not found"
  exit 1
fi
...
rm -f provider/cmd/pulumi-resource-redpanda/bridge-metadata.json

# Go modules & code
replace_go_src() {
  replace "${1}" github.com/pulumi/terraform-provider-redpanda "${upstream}"
  replace "${1}" "github\.com/pulumi/pulumi-redpanda" "github\.com/${organization}/${repository}"
  replace "${1}" xyz "${provider}"
}
replace provider/go.mod "github\.com/pulumi/pulumi-redpanda" "github\.com/${organization}/${repository}"
replace provider/go.mod "github.com/pulumi/terraform-provider-redpanda v.*" "${upstream} latest"
replace_go_src provider/resources.go
replace_go_src provider/cmd/pulumi-tfgen-redpanda/main.go
replace_go_src provider/cmd/pulumi-resource-redpanda/main.go
if [[ "${provider}" != "xyz" ]]; then # Don't fail on no-op rename
  mv provider/cmd/pulumi-tfgen-redpanda "provider/cmd/pulumi-tfgen-${provider}"
  mv provider/cmd/pulumi-resource-redpanda "provider/cmd/pulumi-resource-${provider}"
fi
# Resolve to a real upstream version in the go.mod
(cd provider && go get "${upstream}")
replace examples/go.mod "github\.com/pulumi/pulumi-redpanda" "github\.com/${organization}/${repository}"
